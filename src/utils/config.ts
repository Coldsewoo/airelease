...
const configParsers = {
  ...
  model(model?: string, otherConfig?: RawConfig | null) {
    ...
    const openaiModels = [
      // Latest (2025) and legacy for compatibility
      'gpt-5',
      'gpt-4.5',
      'gpt-4.1',
      'gpt-4o',
  ...
  async editor(editor?: string) {
    ...
    if (!isInstalled) {
      console.warn(`Warning: '${editor}' does not appear to be installed or is not in your PATH.`)
      ...
      const availableEditors = installedEditors.filter((ed) => ed.available).map((ed) => ed.name)

      if (availableEditors.length > 0) {
        console.warn(`Available editors on your system: ${availableEditors.join(', ')}`)
        console.warn(
...
export const setConfigs = async (keyValues: [key: string, value: string][]) => {
  ...
  for (const [key, value] of keyValues) {
    ...
    try {
      ...
      const parsed = await Promise.resolve(configParsers[key as ConfigKey](value))
